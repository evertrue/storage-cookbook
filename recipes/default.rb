...
if node['storage'] == {}
  if node['ec2'] &&
    ...
    if mnt_device.nil?
      Chef::Log.debug 'No /mnt devices found in node[:filesystem]:'
      Chef::Log.debug node['filesystem'].inspect
    else
      mount '/mnt' do
    ...
    Chef::Log.debug 'Discovered ephemeral devices: ' + dev_names.join(', ')
  else
    Chef::Log.debug('Not a recognized hypervisor type.  Not mounting any volumes.')
  end
